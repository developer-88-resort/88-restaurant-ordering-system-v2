...
export default function Index({ items, categories, hasCategories, showArchived, archivedCount, filters, availabilityOptions, auth }) {
    ...
    const [statuses, setStatuses] = useState(() => Object.fromEntries(items.data.map((i) => [i.id, i.availability_status])));

    useEffect(() => {
        setStatuses(Object.fromEntries(items.data.map((i) => [i.id, i.availability_status])));
    }, [items.data]);
            ...
            {items.data.length === 0 ? (
                ...
                <>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                        {items.data.map((item) => (
                            <div key={item.id} className="group bg-white border border-[#E5DDD0] rounded-xl overflow-hidden flex flex-col shadow-sm hover:shadow-md hover:border-[#D9CCBA] transition-all duration-200">
                                <div className="aspect-square bg-gradient-to-br from-[#FAF6EE] to-[#F1E9DA] relative overflow-hidden">
                                    ...
                                    {showArchived ? (
                                        ...
                                        canManageMenu && (
                                            <div className="absolute top-1.5 right-1.5">
                                                <AvailabilityMenu item={item} status={statuses[item.id]} options={options} onSelect={selectAvailability} />
                                            </div>
                                        )
                                    )}
                                </div>
                                ...
                                <div className="p-2.5 flex-1 flex flex-col">
                                    <span className="text-[9px] font-semibold text-[#8A7B9E] uppercase tracking-wider">{item.category_name}</span>
                                    <h3 className="mt-0.5 text-sm font-semibold text-gray-900 leading-snug truncate" title={item.name}>
                                        {item.name}
                                    </h3>
                                    {item.description && (
                                        <p className="text-[10px] text-gray-500 truncate" title={item.description}>
                                            {item.description}
                                        </p>
                                    )}
                                    ...
                                    <div className="mt-auto pt-2 flex items-end justify-between">
                                        <span className="text-sm font-bold text-[#8A3330]">{item.price_range_label}</span>
                                        {canManageMenu && (
